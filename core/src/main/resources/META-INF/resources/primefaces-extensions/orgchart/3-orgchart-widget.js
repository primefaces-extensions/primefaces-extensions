 ...
 * @since 6.3
 */
var PFE_ORGCHART_FILTER_DEFAULTS = Object.freeze({
...
var PFE_ORGCHART_FILTER_ROLES = Object.freeze({
    input: 'filter-input',
    apply: 'filter-apply',
    clear: 'filter-clear'
});
...
PrimeFaces.widget.ExtOrgChart = PrimeFaces.widget.BaseWidget.extend({
     ...
     */
    filterNodes: function (keyWord, showAlert) {
     ...
     */
    getOrgChartRoot: function () {
        return this.jq.children('.orgchart').first();
    },

    _ensureFilterControls: function () {
    ...
    _unbindFilterControls: function () {
        var $panel = this.jq.children('.pfe-orgchart-filter-controls');
        if (!$panel.length) {
            return;
        }
    ...
    _loopVisibleHierarchies: function ($hierarchy) {
        if (!$hierarchy.length) {
            return;
        }
