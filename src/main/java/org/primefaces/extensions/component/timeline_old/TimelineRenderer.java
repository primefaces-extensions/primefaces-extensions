 ...
 * $Id$
 */
package org.primefaces.extensions.component.timeline_old;
...
public class TimelineRenderer extends CoreRenderer {
	...
	protected void encodeScript(final FacesContext context, final Timeline component) throws IOException {
		...
		if (component.getValue() instanceof org.primefaces.extensions.model.timeline_old.Timeline) {
			org.primefaces.extensions.model.timeline_old.Timeline timeline =
			    (org.primefaces.extensions.model.timeline_old.Timeline) component.getValue();
			...
			List<org.primefaces.extensions.model.timeline_old.Timeline> model =
			    (List<org.primefaces.extensions.model.timeline_old.Timeline>) component.getValue();
			if (!model.isEmpty()) {
				String groupName;
				boolean hasGroup = model.size() > 1;
				writer.write(",dataSource: [");
				for (Iterator<org.primefaces.extensions.model.timeline_old.Timeline> it = model.iterator(); it.hasNext();) {
					org.primefaces.extensions.model.timeline_old.Timeline timeline = it.next();
